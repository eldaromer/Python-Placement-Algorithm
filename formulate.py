...
# This is an update over the old one. It operates on grid coordinates rather than FGU
# coordinates to save time, and does not generate duplicates (no need for set)
# The technique is to bloat bb1's envelope by bb2's envelope and search in that.
# This function basically implements "permute" but for grid coordinates.
def findCollisions(bb1, bb2):

    # First, calculate the search area in grid coordinates
    # This does little for small workspaces, but saves much time for large ones.
    # Fraught with potential fence problems.
    collisions = []
    bb1.reHome()
    bb2.reHome()
    bb2Radius = max([bb2.maxGridX(), bb2.maxGridY(),
                     -bb2.minGridX(), -bb2.minGridY()])
    xRange = range(bb1.minGridX() - bb2Radius - 1, bb1.maxGridX() +  bb2Radius + 2)
    yRange = range(bb1.minGridY() - bb2Radius - 1, bb1.maxGridY() +  bb2Radius + 2)
    zRange = range(bb1.minGridZ() - bb2.maxGridZ() - 1, bb1.maxGridZ() - bb2.minGridZ() + 2)

    for x, y, z, r in product(xRange, yRange, zRange, range(4)):
        bb2.translateTo(grid2origin((x, y, z)))
        bb2.rotateTo(r)
        collision = False
        for fgu1 in bb1.FGU_list:
            if fgu1.gender is None:
                ok = 0
            else:
                ok = not fgu1.gender
            gc = lambda x,y: (x.align is not y.align) or (y.gender is not ok)
            for fgu2 in bb2.FGU_list:
                if fgu1.position != fgu2.position:
                    continue
                elif (fgu1.align is not fgu2.align) or (fgu2.gender is not ok):
                    collision = True
                    break
            if collision:
                collisions.append([bb2.origin, bb2.orientation])
                break
    return collisions
...
def match2props(bb1, bb2, matches, b2p, ws):
    ...
    for m in matches:
        ...
        for rot in [1, 2, 3, 0]:
            ...
            bb1.globalRotate(1)
            bb2.globalRotate(1)
            env = bb1.mergedEnvelope(bb2)
            minc = env[0]
            maxc = env[1]
            
            for i in range(3):
                freedom = ws[i] - (maxc[i] - minc[i])
...
def autosolve(schematic, tries = 3):
...
def solve(schematic):
    ...
    log.append(str(ws[0]) + '-' + str(ws[1]) + '-' + str(ws[2]))
    log.append(len(bb))
    log.append(schematic.countFGU())
    log.append(len(cn))
    ...
    log.append(len(uniquenessClauses))
    log.append(str(time() - start))
    ...
    for combo in combinations(bb, 2):
        ...
        sTime.append(time() - start)

    print str(len(collisions)) + " collision clauses generated."
    print str(len(shorts)) + " short clauses generated."
    log.append(len(collisions))
    log.append(str(sum(cTime)))
    log.append(len(shorts))
    log.append(str(sum(sTime)))
    ...
    connectTime = time() - start
    print str(len(connectClauses)) + " connection clauses generated."
    log.append(len(connectClauses))
    log.append(connectTime)
    ...
    for f in schematic.fixed:
        fixed.append([b2p[f]])
    log.append(len(fixed))
    write_file(log)

    # Great, that's all the requirements. Concatenate them here, and call PicoSAT.
    gc.collect()
    cnf = []
    cnf.extend(exclusivePairs)
    del exclusivePairs
    cnf.extend(connectClauses)
    del connectClauses
    cnf.extend(uniquenessClauses)
    del uniquenessClauses
    cnf.extend(fixed)
    del fixed
    ...
    write_file([solveTime])
    print "Solution took " + str(solveTime) + " seconds."
    if str(soln) == 'UNSAT':
